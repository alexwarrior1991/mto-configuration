-- Amplia async_job_type_check para admitir el nuevo JobType.MASTER_DATA_REPUBLISH.
--
-- Mismo motivo que V10 y V13: ddl-auto: validate NO comprueba los CHECK, asi que sin esta
-- migracion el fallo no saldria al arrancar sino en el primer INSERT, al lanzar el primer
-- republicado de datos maestros, y ya en produccion.
--
...
ALTER TABLE async_job ADD CONSTRAINT async_job_type_check
    CHECK (job_type IN ('PROFILE_EXPORT', 'PROFILE_BULK_CREATE', 'PROFILE_BULK_UPDATE',
                        'LOV_IMPORT', 'PROFILE_IMPORT', 'MASTER_DATA_REPUBLISH'));
